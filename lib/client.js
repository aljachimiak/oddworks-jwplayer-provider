...
class Client {
	...
	constructor(spec) {
		...
		this.getConversionsByVideo = this.getConversionsByVideo.bind(this);
	}
	...
	getPlaylists(args) {
		...
		// max is 1000
		args.query = {
			result_limit: 500  // eslint-disable-line camelcase
		};
	...
	getVideosByPlaylist(args) {
		...
		args.query.channel_key = id; // eslint-disable-line camelcase
		console.log('making feed request');
		return this.makeRequest(args);
	}

	// arge.playlistId *requred*
	// args.apiKey
	// args.secretKey
	getFeed(args) {
		const id = args.playlistId;
		if (!id || typeof id !== 'string') {
			throw new Error('getFeed() playlistId is required');
		}
		args.path = `/feed.json`;
		args.query = args.query || {};
		args.query.channel_key = id; // eslint-disable-line camelcase
		...
		args.query.token = Client.generateFeedJWT(jwtArgs);
		return this.makeRequest(args);
	}
	...
	getConversionsByVideo(args) {
		...
		args.query = args.query || {};
		args.query.video_key = id; // eslint-disable-line camelcase
		return this.makeRequest(args);
	}
	...
	makeRequest(args) {
		...
		const url = (path.indexOf('/feed.json') > -1) ? `https://content.jwplatform.com${path}` : `${this.baseUrl}${path}`;
	...
	static request(params) {
		return new Promise((resolve, reject) => {
			request(params, (err, res, body) => {
				if (err) {
					debug(`Error in JWPlayer provider client: ${err}`);
					return reject(err);
				}

				if (res.statusCode === 404) {
					console.log('404 - typeof(res.body): ', typeof (res.body));
					debug(`404 response in JWPlayer provider client (res.body): ${res.body}`);
					return resolve(null);
				}
	...
	// args.playlidId *required
	// args.secretKey *required
	// args.videoId optional
	static generateFeedJWT(args) {
		const secretKey = args.secretKey || '';
		const feed_id = args.playlistId || '';
		const related_media_id = args.videoId || '';

		if (secretKey.length === 0) {
			console.error('generateFeedJWT requires args.secretKey');
			return '';
		}
		if (feed_id.length === 0) {
			console.error('generateFeedJWT requires args.playlistID');
			return '';
		}
		...
		const exp = Math.round(time.getTime() / 1000);
		const resource = 'feed.json';
		...
		if (related_media_id.length > 0) {
			payload.related_media_id = related_media_id;
		}
